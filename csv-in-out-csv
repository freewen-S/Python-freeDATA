...
for root, dirs, files in os.walk('all_trading_data/stock data'):  # 注意：这里请填写数据文件在您电脑中的路径；前继路径为当前工作路径
    if files:
        for f in files:
            if '.csv' in f:  # 识别文件类型；f为文件名，含文件类型名
                stock_code_list.append(f.split('.csv')[0])  #取f不含文件类型的文件名
...
for code in stock_code_list:
    ...
    if code[2] != '3':
        continue
    # 从csv文件中读取该股票数据
    stock_data = pd.read_csv('all_trading_data/stock data/' + code + '.csv',   # 注意：这里请填写数据文件在您电脑中的路径
                                parse_dates=[1])
